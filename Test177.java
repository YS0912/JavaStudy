...
○ 객체 직렬화(Object Serialization)
	...
	멤버 변수 값을 보존한다거나 다른 네트워크에 있는 호스트에 값을 보낼 경우 사용하게 된다.

	즉, 객체 직렬화는 내용물을 바이트 단위로 변환하여
	파일 또는 네트워크를 통해 송수신(스트림)이 가능하게 만들어주는 것으로
	이때, 객체란 멤버 변수의 메모리만으로 구성된 것을 말한다.

	따라가서 메소드와 생성자는 객체 직렬화의 대상에서 제외된다.

○ 객체 직렬화의 장점
	
	객체 자체의 내용을 입출력 형식에 구애받지 않고
	객체를 파일에 저장함으로써 영속성(영구적으로 보존되는 속성)을 제공할 수 있으며
	객체 자체를 네트워크를 통해 손쉽게 교환할 수 있게 된다.

	객체 직렬화는 자바 1.1 이후에 도입되었는데 그 이유는 RMI와 Bean 때문이었다.
	RMI는 원래 객체 통신을 지원해야 하기 때문에 객체가 그대로 이동할 수 있어야 한다.
...
○ Serializable 인터페이스
	...
	이 인터페이스는 객체 직렬화가 제공되어야 한다는 사실을 JVM에 알려주는 역할을 수행한다.
	또한, Serializabel 인터페이스는 다른 인터페이스와 달리
	구현해야 할 메소드가 없기 때문에 단지 선언만 해주면 된다.
	(추상 메소드를 오버라이딩 해야 할 필요가 없기 때문에 추상은 언제든 떨어질 수 있다.)
	...
	ObjectInpuStream 클래스와 ObjectOutputStream 클래스를 이용하여 객체 단위로 입출력을 수행하게 된다.

	★ 멤버 변수가 static으로 선언된 경우 (즉 클래스 변수일 경우) 객체 직렬화 대상에서 제외된다.

○ Object Stream

	java.io.ObjectInputStream
	ObjectInputStream 클래스는 ObjectOutputStream 클래스에 의해 파일에 저장된 있는 객체나 네트워크를 통해
	전달된 객체의 직렬화를 해제하는 기능을 제공한다.
	...
	이때, readObjec() 메소드를 이용하여 스트림으로부터 직렬화된 객체를 읽을 수 있으며
...
public class Test177
{
	public static void main(String[] args) throws Exception
	{
		...
		// appDir ▷ C:\JavaStudy
		//	-- appDir 위치를 기준으로 "\\data\\test.ser"를 구성하겠다는 의미
		...
		//	-- 파일 전용 출력 스트림에 f0라는 객체를 띄우겠다!
		//	   ▷ InputStreamReader isr = new InputStreamReader(System.in);
		FileOutputStream fos = new FileOutputStream(f0);


		// 객체 전용 출력 스트림 생성
		//	-- 객체 전용 출력 스트림으로 fos라는 파일 전용 스트림을 감싸겠다!
		//	   ▷ BufferedReader br = new BufferedReader(isr);
		ObjectOutputStream oos = new ObjectOutputStream(fos);

		// 위의 line 136 + 142 과 구조적으로 동일한 구문
		//	▷ BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		//	▶ ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(f0));

		// ObjectOutputStream 클래스는 객체들을 출력하는 기능을 제공하는 클래스로
		...
		if (f0.exists())
		{
			...
			// 객체 입력 스트림(ois, ObjectInputStream)로 읽어들인 객체(Object)를
			// 캐스팅(Hashtable) 하여 h2라는 Hashtable 자료구조에 담아내기
