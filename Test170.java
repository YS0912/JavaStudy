...
○ Hashtable<K, V> 클래스
	...
	- 해시테이블이란 키(key)와 그에 상응하는 데이터(value)로 구분된
	  데이터 구조이다.
...
public class Test170
{
	...
	public static void main(String[] args)
	{
		...
		// ht라는 Hashttable 자료구조에서 key를 이용하여 데이터 검색
		// ▷ get(k);
		...
		// ht라는 Hashtable 자료구조에 key가 존재하는지 여부 확인
		// ▷ containsKey()
		...
		// ht라는 Hashtable 자료구조에서 『반보영』 데이터 삭제
		// ▷ remove()
		ht.remove("반보영");

		// remove() 메소드는 인자값으로 key를 넘겨받지만 이때 key만 삭제되는 것이 아니다.
		...
		if (ht.containsKey("반보영"))
			...
			System.out.println("보영이 어디 갔어??");
		System.out.println();
		// → 보영이 어디 갔어??

		
		// 삭제 이후 value가 존재하는지 확인
		...
		//	  기존 "010-2546-0266"dptj "010-1111-1111"로 변경되었음을 확인
		//	  (덮어쓰기 개념)
		...
		// value는 중복된 값이 입력되더라도 key를 기준으로 검색하는데 문제가 없다.
	}
}