...
○ 캐릭터 셋(Character Set)
	...
	▶ 유니코드(unicode)
		국제 표준으로 제정된 2바이트계의 만국 공통의 국제 문자부호 체계(UCS: Universal Code System)
		애플컴퓨터, IBM, 마이크로소프트 등이 컨소시엄으ㅗ 설립한 유니코드(Unicode)가
		1990년에 첫 버전을 발표하였고 ISO/IEC JTC1에서 1995년 9월 국제표준으로 제정하였다.
		공식 명칭은 ISO/IEC 10646-1(Univesal Multiple-Octer Coded Character Set)이다.

		데이터의 교환을 원활하게 하기 위하여 문자 1개에 부여되는 값을 16비트로 통일했다.
		코드 1문자 당 영어는 7비트, 비영어는 7비트, 한글이나 일본어는 16비트의 값을 가지는데,
		이들을 모두 16비트로 통일한 것이다.

	▷ UTF-8과 UTF-16
		UTF-8과 UTF-16의 차이를 한 마디로 말하면 문자를 표현할 때의 단위가 다르다는 것이다.
		...
		- UTF-16(16bit UCS Transformation Format)
		  자바의 기본 코드
		  자바에서는 문자 하나를 저장하면 바이트 수는 영문자든, 한글문자든 2바이트를 차지한다.
		  UCS-2로 정의된 문자 집합을 이용하여 기술된 문자열에 UCS-4의 일부 문자를 채우기 위한 인코딩 방식
		  UTF-8과 병용할 수 있다.
...
○ 문자 코드의 종류
	...
	▷ KSC5601
		한글 완성형 표정(한글 2,350자 표현)
		한국공업표중 정보처리분야(C)의 5601번 표준안
	...
	▶ EUC-KR(MS949) = ANSI
		Bell-Laboratories에서 유닉스 상에서 영문자 이외의 문자를 지원하기 위해 제안한
		확장 유니코드(Extend UNIX Code) 중 한글 인코딩 방식
		영문은 KSC5636으로 처리하고, 한글은 KSC5601로 처리
		즉, EUC-KR(MS949) = KSC5636 + KSC5601

	▶ UTF-8
		영문과 숫자는 1바이트, 한글은 3바이트로 표현
		(웹에서도 많이 사용, UFF-8 형식으로 JSP를 작성할 때에는 파일도 UTF-8 형식으로 저장해야 한다.
	...
	▷ ISO-8859-1
		서유럽 언어 표기에 필요한 US-ASCII에 없는 94개 글자의 순차적 나열
*/
...
public class Test141
{
	public static void main(String[] args) throws UnsupportedEncodingException
	{
		...
		// → 컴파일 에러: UnsupportedEncodingException

		// ▶ 예외 처리 후 다시 확인
		System.out.println("lenght : " + buf.length);
		// → lenght : 9
		...
		// utf-8 형태로 저장된 바이트 배열을
		// String 객체로 파라미터 값 없이 디폴트 캐릭터셋(MS949)을 부여하여 생성
		...
		/* ★
			...
			euc-kr을 utf-8로 변환하겠다는 의도로 위와 같이 코딩을 해버리면
			이러한 변환(잘못된 변환)으로 인해 배열이 깨져서
			euc-kr이든, utf-8이든 어떠한 바이트 배열로 읽어오든
			결과물이 깨져있는 것을 확인할 수 있다.

			이미 깨져서 생성된 String 객체의 바이트 배열은 어떤 방식으로든 복구 불가능
		*/
	}
}