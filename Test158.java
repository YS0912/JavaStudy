...
○ 벡터(Vector)
	...
	배열과 달리 다른 종류의 데이터형을 가진 데이터 요소들을 가질 수 있으며,
	데이터가 가득 차면 자동으로 저장 영역을 확장시키게 된다.

	또한 객체 레퍼런스를 저장하기 때문에
	JDK 1.5 이전에는 자바 기본형 데이터는 저장할 수 없었으나
	JKD 1.5 이후부터는 오토박싱 / 오토언박싱 기능이 지원되면서
	기본 데이터형도 저장이 가능하다.

	벡터 클래스의 기본 구조는
	시퀸스(Sequence) 데이터 구조에 기반하며
	시퀸스 데이터 접근을 순차적인 인덱스 값에 대한 위치로 접근하게 된다.
	따라서 자료구조의 양 끝단에서만 접근이 가능한 큐나 한쪽 끝에서만 접근이 가능한 스택과 달리
	시퀸스 캐념의 인덱스 값을 이용하여 자료 구조의 임의의 지점에서 저장 및 접근이 가능하다.

	벡터의 시퀸스 구조는 List 인터페이스를 통해 제공받고 있기 때문에
